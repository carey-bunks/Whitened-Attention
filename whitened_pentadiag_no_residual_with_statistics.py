...
hparams = {
    ...
    'stats_file': '4x4_stats_pentadiag_whiten_no_residual.txt'
}
...
@torch.no_grad()
def estimate_ar_stationarity(x):
    """
    Estimates if sequences in x (B x T x C) are from stationary AR(1) and AR(2) processes.
    Computes first-order (lag-1) and second-order (lag-2) C x C sample cross-covariance matrices 
    across batch dimension and measures their variation along the sequence dimension T.
    
    Args:
        x (torch.Tensor): Input tensor of shape (B, T, C)
    
    Returns:
        dict: Contains results for 'lag1' and 'lag2', each with mean covariance matrix, 
              variance of covariance matrices along T, and mean variation.
    """
    B, T, C = x.shape
    ...
    # Initialize tensors to store covariance matrices for lag-1 and lag-2
    cov_matrices_lag1 = torch.zeros(T-1, C, C, device=x.device)
    cov_matrices_lag2 = torch.zeros(T-2, C, C, device=x.device) if T > 2 else None
    
    # Compute sample cross-covariance matrices for lag-1
    for t in range(T-1):
        x_t = x_centered[:, t, :]      # Shape: (B, C)
        x_t_next = x_centered[:, t+1, :]  # Shape: (B, C)
        cov_matrices_lag1[t] = torch.matmul(x_t.transpose(-1, -2), x_t_next) / (B - 1)
    
    # Compute sample cross-covariance matrices for lag-2 (if possible)
    if T > 2:
        for t in range(T-2):
            x_t = x_centered[:, t, :]      # Shape: (B, C)
            x_t_next2 = x_centered[:, t+2, :]  # Shape: (B, C)
            cov_matrices_lag2[t] = torch.matmul(x_t.transpose(-1, -2), x_t_next2) / (B - 1)
    
    # Compute mean covariance matrices
    mean_cov_lag1 = cov_matrices_lag1.mean(dim=0)  # Shape: (C, C)
    mean_cov_lag2 = cov_matrices_lag2.mean(dim=0) if cov_matrices_lag2 is not None else None
    
    # Compute variation for lag-1
    cov_diff_lag1 = cov_matrices_lag1 - mean_cov_lag1
    cov_variation_lag1 = torch.norm(cov_diff_lag1, p='fro', dim=(-2, -1))  # Shape: (T-1,)
    mean_variation_lag1 = cov_variation_lag1.mean()
    
    # Compute variation for lag-2 (if possible)
    if cov_matrices_lag2 is not None:
        cov_diff_lag2 = cov_matrices_lag2 - mean_cov_lag2
        cov_variation_lag2 = torch.norm(cov_diff_lag2, p='fro', dim=(-2, -1))  # Shape: (T-2,)
        mean_variation_lag2 = cov_variation_lag2.mean()
    else:
        cov_variation_lag2 = None
        mean_variation_lag2 = None
    
    return {
        'lag1': {
            'mean_cov_matrix': mean_cov_lag1,
            'cov_variation': cov_variation_lag1,
            'mean_variation': mean_variation_lag1
        },
        'lag2': {
            'mean_cov_matrix': mean_cov_lag2,
            'cov_variation': cov_variation_lag2,
            'mean_variation': mean_variation_lag2
        } if T > 2 else None
    }
...
def evaluate_whiteness(x, w):
    ...
    ar1_x = estimate_ar_stationarity(x)
    return {
        'whiteness_x': whiteness_x,
        'whiteness_w': whiteness_w,
        'is_w_whiter': is_whiter,
        'ar1_x': ar1_x['lag1']['mean_variation'],
        'ar2_x': ar1_x['lag2']['mean_variation']
    }
...
class BlockPentadiagonalWhitening(nn.Module):
    def __init__(self):
        super().__init__()
        self.V_0 = nn.Parameter(torch.tril(torch.randn(n_embd, n_embd)*.01), requires_grad=True)
        self.V_1 = nn.Parameter(torch.randn(n_embd, n_embd)*.01, requires_grad=True)
        self.V_2 = nn.Parameter(torch.randn(n_embd, n_embd)*.01, requires_grad=True)
        
    def forward(self, x: torch.Tensor, compute_whiteness: bool = True) -> torch.Tensor:
        """
        Compute whitened tensor w using the recursive formula:
        w_0 = V_0 * x_0
        w_1 = V_0 * (x_1 - V_1 * w_0)
        w_i = V_0 * (x_i - V_1 * w_{i-1} - V_2 * w_{i-2}) for i = 2,...,T-1
        """
        B, T, C = x.shape
        x = x - x.mean(dim=-1, keepdim=True)
        w = torch.zeros_like(x)
        w[:, 0, :] = torch.matmul(x[:, 0, :], self.V_0)
        prev_term_1 = torch.matmul(x[:, 0, :], self.V_1)
        diff_1 = x[:, 1, :] - prev_term_1
        w[:, 1, :] = torch.matmul(diff_1, self.V_0)
        for i in range(2, T):
            prev_term_1 = torch.matmul(w[:, i-1, :], self.V_1)
            prev_term_2 = torch.matmul(w[:, i-2, :], self.V_2)
            diff = x[:, i, :] - prev_term_1 - prev_term_2
            w[:, i, :] = torch.matmul(diff, self.V_0)
        if compute_whiteness:
            results = evaluate_whiteness(x, w)
            return w, results
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head):
        ...
        self.whitener = BlockPentadiagonalWhitening()
    ...
    if compute_whiteness and whitening_results:
        with open(stats_file, 'a') as f:
            for i, results in enumerate(whitening_results):
                f.write(f"Iter {iter} | Block {i+1} | x: {results['whiteness_x'].item():.4f} | w: {results['whiteness_w'].item():.4f} | r: {results['whiteness_w'].item()/results['whiteness_x'].item():.4f} | ar1: {results['ar1_x'].item():.4f} | ar2: {results['ar2_x'].item():.4f}\n")
